...
pub use frame_support::{
	...
	weights::Weight,
	Blake2_256, IterableStorageDoubleMap, IterableStorageMap, PalletId, StorageDoubleMap,
...
decl_storage! {
	trait Store for Module<T: Config> as ListenModule {
		...
		pub GroupId get(fn group_id): u64 = 1;
		...
		pub InfoOfDisbandedRoom get(fn info_of_disband_room): double_map hasher(blake2_128_concat) SessionIndex, hasher(blake2_128_concat) u64 => RoomRewardInfo<BalanceOf<T>>;
		...
		pub Depth get(fn depth): u32 = 20;

		/// Vote on dissolution has been passed by the group
		///
		/// (如果要查询投票是否还在进行 通过room_info来查，并且配合投票过期时间)
		pub DisbandingRooms get(fn disband_rooms): BTreeMap<u64, T::BlockNumber>;
		pub LastSessionIndex get(fn last_session_index): u32;
		pub NowSessionIndex get(fn session_index): u32;

	}
}

decl_error! {
	/// Error for the electionsLastSessionIndex module.
...
decl_module! {
	...
	pub struct Module<T: Config> for enum Call where origin: T::Origin {
		...
		/// How long the red envelope expires
		const RedPackExpire: T::BlockNumber = T::RedPackExpire::get();
		/// How often to reward the group leader.
		...
		/// Duration of group protection
		const ProtectedDuration: T::BlockNumber = T::ProtectedDuration::get();
		/// The maximum number of members of a group council.
		const CouncilMaxNumber: u32 = T::CouncilMaxNumber::get();
		/// Delay duration of group disbanding
		const DelayDisbandDuration: T::BlockNumber = T::DelayDisbandDuration::get();
		/// LT currency id in the tokens module
		...
		#[weight = 10_000]
		fn set_multisig(origin, members: Vec<<T::Lookup as StaticLookup>::Source>, threshould: u16){
			...
			let multisig_id = <pallet_multisig::Module<T>>::multi_account_id(&members, threshould.clone());
		...
		#[weight = 10_000]
		fn set_server_id(origin, server_id: <T::Lookup as StaticLookup>::Source) {
			ensure_root(origin)?;

			let server_id = T::Lookup::lookup(server_id)?;
			<ServerId<T>>::put(server_id.clone());

			Self::deposit_event(RawEvent::SetServerId(server_id));
		}
		...
		#[weight = 10_000]
		fn air_drop(origin, members: Vec<<T::Lookup as StaticLookup>::Source>) -> DispatchResult {
		...
		fn manager_get_reward(origin, group_id: u64) {
			...
			let mut duration_num = time.checked_div(&T::RewardDuration::get()).ok_or(Error::<T>::DivByZero)?;
		...
		fn update_join_cost(origin, group_id: u64, join_cost: BalanceOf<T>) -> DispatchResult{
			...
			let mut room_info = <AllRoom<T>>::get(group_id).ok_or(Error::<T>::RoomNotExists)?;
			ensure!(room_info.join_cost.clone() != join_cost.clone(),  Error::<T>::AmountShouldDifferent);
		...
		#[weight = 10_000]
		fn into_room(origin, group_id: u64, invitee: Option<<T::Lookup as StaticLookup>::Source>) -> DispatchResult{
			...
			if room_info.is_private.clone() {
				ensure!(invitee.is_some() && inviter.clone() == room_info.group_manager.clone(), Error::<T>::PrivateRoom);
			}
			...
			Self::deposit_event(RawEvent::IntoRoom(invitee, inviter, group_id));
			Ok(())
		}
		...
		fn buy_props_in_room(origin, group_id: u64, props: AllProps) -> DispatchResult{
			...
			ensure!(props.picture != 0u32 || props.text != 0u32 || props.video != 0u32, Error::<T>::BuyNothing);
		...
		fn buy_audio_in_room(origin, group_id: u64, audio: Audio) -> DispatchResult{
			...
			let mut dollars = <BalanceOf<T>>::from(0u32);
			ensure!(audio.ten_seconds != 0u32 || audio.thirty_seconds != 0u32 || audio.minutes != 0u32, Error::<T>::BuyNothing);
		...
		#[weight = 10_000]
		fn remove_someone_from_blacklist(origin, group_id: u64, who: <T::Lookup as StaticLookup>::Source) {
			T::RoomRootOrHalfCouncilOrigin::try_origin(origin).map_err(|_| Error::<T>::BadOrigin)?;
		...
		/// The Origin must be RoomCouncil or RoomManager.
		#[weight = 10_000]
		fn remove_someone(origin, group_id: u64, who: <T::Lookup as StaticLookup>::Source) -> DispatchResult {
			T::RoomRootOrHalfRoomCouncilOrSomeRoomCouncilOrigin::try_origin(origin.clone()).map_err(|_| Error::<T>::BadOrigin)?;
		...
		fn ask_for_disband_room(origin, group_id: u64) -> DispatchResult{
			...
			let now = Self::now();
			ensure!(now - create_block > T::ProtectedDuration::get(), Error::<T>::InProtectedDuration);
			...
			room.disband_vote.approve_total_amount = room.disband_vote.approve_total_amount.checked_add(&user_consume_amount).ok_or(Error::<T>::Overflow)?;
		...
		#[weight = 10_000]
		fn vote(origin, group_id: u64, vote: ListenVote) -> DispatchResult{
			let who = ensure_signed(origin)?;
			...
			match vote {
				ListenVote::Approve => {
					...
					if room.disband_vote.reject_man.get(&who).is_some() {
						...
						room.disband_vote.reject_total_amount = room.disband_vote.reject_total_amount.saturating_sub(user_consume_amount);
					}
				},
				ListenVote::Reject => {
					if room.disband_vote.reject_man.get(&who).is_some(){
					...
					room.disband_vote.reject_total_amount = room.disband_vote.reject_total_amount.checked_add(&user_consume_amount).ok_or(Error::<T>::Overflow)?;
					if room.disband_vote.approve_man.get(&who).is_some() {
						room.disband_vote.approve_total_amount = room.disband_vote.approve_total_amount.saturating_sub(user_consume_amount);
		...
		fn pay_out(origin) -> DispatchResult {
			...
			ensure!(<AllListeners<T>>::contains_key(who.clone()) && !<AllListeners<T>>::get(who.clone()).rooms.is_empty(), Error::<T>::NotInAnyRoom);
					...
					if !<AllRoom<T>>::contains_key(group_id){
						...
						for i in 0..Self::depth() as usize {
							...
							if <InfoOfDisbandedRoom<T>>::contains_key(cur_session, group_id) {
								...
								info.already_get_reward = info.already_get_reward.saturating_add(reward.clone());
								<InfoOfDisbandedRoom<T>>::insert(cur_session, group_id, info.clone());
		...
		#[weight = 10_000]
		fn give_back_expired_redpacket(origin, group_id: u64) {
			let who = ensure_signed(origin)?;
		...
		#[weight = 10_000]
		pub fn get_redpacket_in_room(origin, amount_vec: Vec<(<T::Lookup as StaticLookup>::Source, BalanceOf<T>)>, group_id: u64, redpacket_id: u128) {
	...
	//			ensure!(amount >= T::RedPacketMinAmount::get(), Error::<T>::AmountTooLow);
				...
				if redpacket.currency_id == T::GetNativeCurrencyId::get() {
					...
					T::MultiCurrency::deposit(redpacket.currency_id.clone(), &who, amount_u128.saturated_into::<MultiBalanceOf<T>>());
				}
		...
		fn on_initialize(n: T::BlockNumber) -> Weight {
			...
			<NowSessionIndex>::put(Self::get_session_index());
			0
			}
...
impl<T: Config> Module<T> {
	fn join_do(who: T::AccountId, invitee: Option<T::AccountId>, group_id: u64) -> DispatchResult {
	...
	fn is_can_disband(group_id: u64) -> result::Result<bool, DispatchError> {
		Self::room_must_exists(group_id)?;
		let disband_rooms = <DisbandingRooms<T>>::get();
		if let Some(disband_time) = disband_rooms.get(&group_id) {
			if Self::now() >= *disband_time {
...
decl_event!(
	pub enum Event<T> where
	 ...
	 Amount = <<T as Config>::NativeCurrency as Currency<<T as system::Config>::AccountId>>::Balance,
	 {
		 ...
		 IntoRoom(Option<AccountId>, AccountId, u64),
