...
fn add_new_vesting_schedule_merges_with_current_locked_balance_and_until() {
	ExtBuilder::build().execute_with(|| {
		...
		assert_eq!(
			...
			Some(&BalanceLock { id: VESTING_LOCK_ID, amount: 17u64, reasons: Reasons::All })
		);
	});
}
...
fn vested_transfer_fails_if_bad_origin() {
	ExtBuilder::build().execute_with(|| {
		let schedule =
			VestingSchedule { start: 0u64, period: 10u64, period_count: 1u32, per_period: 100u64 };
		assert_noop!(Vesting::vested_transfer(Origin::signed(CHARLIE), BOB, schedule), BadOrigin);
	});
}
...
fn claim_for_works() {
	ExtBuilder::build().execute_with(|| {
		...
		assert_eq!(
			PalletBalances::locks(&BOB).get(0),
			Some(&BalanceLock { id: VESTING_LOCK_ID, amount: 20u64, reasons: Reasons::All })
		);
		assert!(VestingSchedules::<Runtime>::contains_key(&BOB));
...
fn update_vesting_schedules_works() {
	ExtBuilder::build().execute_with(|| {
		...
		assert_ok!(Vesting::update_vesting_schedules(Origin::root(), BOB, vec![updated_schedule]));
		...
		assert_eq!(
			PalletBalances::locks(&BOB).get(0),
			Some(&BalanceLock { id: VESTING_LOCK_ID, amount: 10u64, reasons: Reasons::All })
		);
...
fn cliff_vesting_works() {
	...
	ExtBuilder::build().execute_with(|| {
		...
		let balance_lock =
			BalanceLock { id: VESTING_LOCK_ID, amount: VESTING_AMOUNT, reasons: Reasons::All };
