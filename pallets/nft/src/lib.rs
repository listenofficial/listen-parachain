...
pub mod module {
	...
	pub type InSaleTokens<T: Config> =
		StorageMap<_, Twox64Concat, T::ClassId, Vec<SaleInfoOf<T>>, ValueQuery>;
	...
	pub type IssuerOf<T: Config> =
		StorageMap<_, Twox64Concat, NftLevel, (T::AccountId, T::ClassId)>;

	/// the user's all tokens
	#[pallet::storage]
	#[pallet::getter(fn tokens_of)]
	pub type TokensOf<T: Config> =
		StorageMap<_, Twox64Concat, T::AccountId, Vec<(T::ClassId, T::TokenId)>, ValueQuery>;

	#[pallet::storage]
	#[pallet::getter(fn no_owner_tokens_of)]
	pub type NoOwnerTokensOf<T: Config> =
		StorageMap<_, Twox64Concat, T::ClassId, Vec<T::TokenId>, ValueQuery>;
...
impl<T: Config> Pallet<T> {
	...
	pub fn do_mint(
		...
		NextTokenId::<T>::try_mutate(class_id, |id| -> Result<T::TokenId, DispatchError> {
			...
			*id = id.checked_add(&One::one()).ok_or(Error::<T>::NoAvailableTokenId)?;
				...
				Classes::<T>::get(class_id).ok_or(Error::<T>::ClassNotFound)?.issuer ==
					issuer.clone(),
				Error::<T>::NotIssuer
			);
			...
			let token_info = TokenInfo { metadata: bounded_metadata, owner: None, data };
	...
	fn do_buy_token(buyer: &T::AccountId, token: (T::ClassId, T::TokenId)) -> DispatchResult {
		Tokens::<T>::try_mutate_exists(token.0, token.1, |token_info| -> DispatchResult {
			...
			let sale_info =
				Self::get_in_sale_token(token.0, token.1).ok_or(Error::<T>::NotInSale)?;
			T::MultiCurrency::transfer(
				T::GetNativeCurrencyId::get(),
				&buyer,
				&sale_info.seller,
