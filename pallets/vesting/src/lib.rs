...
const VESTING_ID: LockIdentifier = *b"ltvest  ";
...
decl_storage! {
	...
	add_extra_genesis {
		config(vesting): Vec<(T::AccountId, T::BlockNumber, T::BlockNumber, BalanceOf<T>, BalanceOf<T>)>;
		build(|config: &GenesisConfig<T>| {
			use sp_runtime::traits::Saturating;

			/// There is money in the account, and vesting from free balances.
			for &(ref who, begin, per_duration_blocks, per_duration_unlock_amount, liquid) in config.vesting.iter() {
				...
				Vesting::<T>::insert(who, VestingInfo {
					locked: locked,
					unlock_amount_of_per_duration: per_duration_unlock_amount,
					block_number_of_per_duration: per_duration_blocks,
...
impl<T: Config> Module<T> {
	...
	fn update_lock(who: T::AccountId) -> DispatchResult {
		...
		let locked_now =
			vesting.locked_at::<T::BlockNumberToBalance>(now, vesting.block_number_of_per_duration);
	...
	BalanceOf<T>: MaybeSerializeDeserialize + Debug,
{
	...
	fn vesting_balance(who: &T::AccountId) -> Option<BalanceOf<T>> {
		if let Some(v) = Self::vesting(who) {
			let now = <frame_system::Module<T>>::block_number();
			let locked_now =
				v.locked_at::<T::BlockNumberToBalance>(now, v.block_number_of_per_duration);
